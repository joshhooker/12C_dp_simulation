...
NonResonantBackgroundProcess::NonResonantBackgroundProcess(const G4String& processName)
        : G4VDiscreteProcess(processName, fHadronic), scattering_energy_(1e6), scattering_length_(1e6) {
    SetProcessSubType(111);
}

NonResonantBackgroundProcess::~NonResonantBackgroundProcess() = default;

G4double NonResonantBackgroundProcess::GetMeanFreePath(const G4Track& aTrack, G4double, G4ForceCondition* condition) {
...
G4VParticleChange* NonResonantBackgroundProcess::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep) {
    ...
    beam_mass_pdg = beam_mass_pdg + 0.002222*CLHEP::amu_c2;
    ...
    target_mass_pdg = target_mass_pdg + 0.000551*CLHEP::amu_c2;
    ...
    light_recoil_mass_pdg = light_recoil_mass_pdg + 0.000545*CLHEP::amu_c2;
    ...
    G4double state_or_background = G4UniformRand();
    ...
    G4double background_amount = cal->GetBackgroundAmount();
    G4double state_amount = 1. - background_amount;

    ///////////////////////////
    // Get Excitation Energy //
    ///////////////////////////

    // Random q-value from -5 to -2
    // excited_energy = G4UniformRand()*3. - 5.;

    // Use NucleonStates to get q-value or excitation energy
    if (state_or_background < state_amount) {
        ...
        // Sample on excited states (exponential function, E5 and E6)
        G4double rand_ex = getEnergy(G4UniformRand());
        // G4double rand_ex = G4UniformRand()*6.;
        excited_energy = floor(rand_ex*1000 + 0.5)/1000.;
        background = true;
    }
    ...
    G4ParticleDefinition* heavy_recoil_def;
    if (background || (!background && !use_energy_qvalue)) {
        heavy_recoil_def = particle_table->GetIonTable()->GetIon(heavy_recoil_charge, heavy_recoil_mass, excited_energy);
    }
    else {
        heavy_recoil_def = particle_table->GetIonTable()->GetIon(heavy_recoil_charge, heavy_recoil_mass, 0.);
    }
    heavy_recoil_mass_dbl = heavy_recoil_def->GetAtomicMass();
    heavy_recoil_mass_pdg = heavy_recoil_def->GetPDGMass();
    heavy_recoil_mass_pdg = 12142.8*MeV + excited_energy;
    ...
    G4double qvalue;
    if (background || (!background && !use_energy_qvalue)) {
        qvalue = beam_mass_pdg + target_mass_pdg - (light_recoil_mass_pdg + heavy_recoil_mass_pdg);
    }
    else {
        qvalue = excited_energy;
    }
    ...
    if (!background) {
        root_file->GetAngularLabStateDist(excited_state_number)->Fill(kinematics.lab_light_theta/deg);
    }
    ...
    sec1->SetUserInformation(new TrackingInformation(energy, cm_energy, kinematics.cm_light_theta,
                                                     ...
                                                     light_recoil_def, heavy_recoil_def, background, excited_state_number));
...
void NonResonantBackgroundProcess::StartTracking(G4Track* track) {
...
G4VParticleChange* NonResonantBackgroundProcess::Decay(const G4Track& aTrack, G4int light_charge, G4int light_mass,
...
uniform_angle_output NonResonantBackgroundProcess::UniformCMAngle(uniform_angle_input input) {
...
uniform_angle_output NonResonantBackgroundProcess::UniformLabAngle(uniform_angle_input input) {
    G4double beam_mass         = input.beam_mass;
    G4double target_mass       = input.target_mass;
    G4double light_recoil_mass = input.light_recoil_mass;
    G4double heavy_recoil_mass = input.heavy_recoil_mass;
...
uniform_angle_output NonResonantBackgroundProcess::CalculateKinematicsLab(uniform_angle_input input, G4double lab_light_theta, G4double lab_light_phi) {
    G4double beam_mass         = input.beam_mass;
    G4double target_mass       = input.target_mass;
    G4double light_recoil_mass = input.light_recoil_mass;
    G4double heavy_recoil_mass = input.heavy_recoil_mass;
...
G4double NonResonantBackgroundProcess::Lorentzian_shoot(G4double mean, G4double gamma) {
    double u;
    do {
        u = G4UniformRand();
    }
