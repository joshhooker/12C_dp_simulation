...
# EX 1 #
########
def run_bayesian_optimization_ex1(ex1, count):
    file_output_name = 'output_raw_ex1_' + str(count) + '.log'
    ...
    # Bounds for states
    pbounds = {'p_ex1': (0., 1.), 'ex2': (0., 4.), 'p_ex2': (0., 1.), 'ex3': (0., 4.), 'p_ex3': (0., 1.), 'ex4': (0., 4.), 'p_ex4': (0., 1.)}

    # Partial function
    func_to_optimize = partial(black_box_ex1, ex1=ex1, count=count)

    # Bayesian Optimizer
    optimizer = BayesianOptimization(f=func_to_optimize, pbounds=pbounds, verbose=2, random_state=42)

    # For saving progress
    logger_name = "./logs_ex1_" + str(count) + ".json"
    logger = JSONLogger(path=logger_name)
    optimizer.subscribe(Events.OPTIMIZATION_STEP, logger)

    # Give it a hint
    optimizer.probe(
        params={"p_ex1": 0.154, "ex2": 3.089, "p_ex2": 0.12, "ex3": 3.6845, "p_ex3": 0.25, "ex4": 3.854, "p_ex4": 0.476},
        lazy=True
    )
...
def black_box_ex1(ex1, p_ex1, ex2, p_ex2, ex3, p_ex3, ex4, p_ex4, count):
    score = run_simulation_ex1(ex1, p_ex1, ex2, p_ex2, ex3, p_ex3, ex4, p_ex4, count)
    return score

def run_simulation_ex1(ex1, p_ex1, ex2, p_ex2, ex3, p_ex3, ex4, p_ex4, count):
    global counter
    file_output_name = 'output_raw_ex1_' + str(count) + '.log'
    ...
    dir_name = 'output/ex1/' + str(counter)
...
def find_bounds_ex1(parameters, base_chi2):
    parameters_ = parameters
    param_value = parameters[0]
    ...
    while abs(current_chi2 - base_chi2) < 4:
        ...
        run_bayesian_optimization_ex1(current_param, count)
        file_ = np.loadtxt('output_raw_ex1_' + str(count) + '.log', skiprows=1)
    ...
    bounds_ex1 = open('bounds_ex1.dat', 'w')
...
# EX 2 #
########
def run_bayesian_optimization_ex2(ex2, count):
    file_output_name = 'output_raw_ex2_' + str(count) + '.log'
    file = open(file_output_name, "w+")
    file.write('Ex1 Energy; Ex1 Prob; Ex2 Energy; Ex2 Prob; Ex3 Energy; Ex3 Prob; Ex4 Energy; Ex4 Prob; MSE; Chi2; Score\n')
    file.close()
    ...
    # Partial function
    func_to_optimize = partial(black_box_ex2, ex2=ex2, count=count)

    # Bayesian Optimizer
    optimizer = BayesianOptimization(f=func_to_optimize, pbounds=pbounds, verbose=2, random_state=42)

    # For saving progress
    logger_name = "./logs_ex2_" + str(count) + ".json"
    logger = JSONLogger(path=logger_name)
    optimizer.subscribe(Events.OPTIMIZATION_STEP, logger)
...
def black_box_ex2(ex1, p_ex1, ex2, p_ex2, ex3, p_ex3, ex4, p_ex4, count):
    score = run_simulation_ex2(ex1, p_ex1, ex2, p_ex2, ex3, p_ex3, ex4, p_ex4, count)
    return score

def run_simulation_ex2(ex1, p_ex1, ex2, p_ex2, ex3, p_ex3, ex4, p_ex4, count):
    global counter
    file_output_name = 'output_raw_ex2_' + str(count) + '.log'
    file = open(file_output_name, 'a+')
...
def find_bounds_ex2(parameters, base_chi2):
    ...
    while abs(current_chi2 - base_chi2) < 4:
        ...
        run_bayesian_optimization_ex2(current_param, count)
...
def find_parameter_errors_fixed():
    ...
    find_bounds_ex1(parameters, base_chi2)
