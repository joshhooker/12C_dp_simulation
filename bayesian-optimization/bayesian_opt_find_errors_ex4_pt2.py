...
# EX 4 #
########
def run_bayesian_optimization_ex4(ex4, count, low):
    if low:
        file_output_name = 'output_raw_ex4_low_' + str(count) + '.log'
    else:
        file_output_name = 'output_raw_ex4_high_' + str(count) + '.log'
    ...
    # Bounds for states
    pbounds = {'ex1': (0., 4.), 'p_ex1': (0., 1.), 'ex2': (0., 4.), 'p_ex2': (0., 1.), 'ex3': (0., 4.), 'p_ex3': (0., 1.), 'p_ex4': (0., 1.)}

    # Partial function
    func_to_optimize = partial(black_box_ex4, ex4=ex4, count=count, low=low)
    ...
    # For saving progress
    if low:
        logger_name = "./logs_ex4_low_" + str(count) + ".json"
    else:
        logger_name = "./logs_ex4_high_" + str(count) + ".json"
    logger = JSONLogger(path=logger_name)
    optimizer.subscribe(Events.OPTIMIZATION_STEP, logger)

    # Give it a hint
    optimizer.probe(
        params={"ex1": 0.0, "p_ex1": 0.154, "ex2": 3.089, "p_ex2": 0.12, "ex3": 3.6845, "p_ex3": 0.25, "p_ex4": 0.476},
        lazy=True
    )
...
def black_box_ex4(ex1, p_ex1, ex2, p_ex2, ex3, p_ex3, ex4, p_ex4, count, low):
    score = run_simulation_ex4(ex1, p_ex1, ex2, p_ex2, ex3, p_ex3, ex4, p_ex4, count, low)
    return score

def run_simulation_ex4(ex1, p_ex1, ex2, p_ex2, ex3, p_ex3, ex4, p_ex4, count, low):
    global counter
    if low:
        file_output_name = 'output_raw_ex4_low_' + str(count) + '.log'
    else:
    ...
    dir_name = 'output/ex4/' + str(counter)
...
def find_bounds_ex4(parameters, base_chi2):
    ...
    bounds_ex2 = open('bounds_ex4.dat', 'a')
